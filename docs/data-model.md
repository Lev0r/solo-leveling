...
Yearly summary. Generated by the user after the calendar year completes. Once an archive exists, raw `dailyLogs` for that year may be deleted to save space and read quotas.
...
|-------|------|----------|-------------|
| `schemaVersion` | number | yes | Currently `1` |
| `year` | number | yes | Matches doc ID |
| `summarizedAt` | ISO 8601 UTC | yes | When the summary was computed |
| `userTimezoneAtSummary` | string (IANA TZ) | yes | Timezone used for date-bucket math |
| `totals.daysTrained` | number | yes | Distinct `dailyLogs` where any exercise was completed |
| `totals.workoutsByDayId` | record | yes | Counts of trained days per routine day id |
| `exerciseTotals.{id}.completedSessions` | number | yes | Times this exercise was marked complete |
| `exerciseTotals.{id}.totalWorkSeconds` | number | no | Sum of timer work seconds (timed exercises only) |
| `streaks.longestTrainedDays` | number | yes | Longest consecutive calendar days with training |
| `streaks.longestRestDays` | number | yes | Longest gap without training within the year (includes year boundaries) |
| `rawLogsDeletedAt` | ISO 8601 UTC \| null | yes | Set when raw logs were deleted; `null` until then |
