...
- [ ] **Font selection** — evolutionary brainstorm in chat: present 5 candidates, user picks 1, propose 5 more close to the pick, repeat until convergence. Final result may use separate families for the logo / headings vs body. Apply via `--font-body` / `--font-display` tokens; no code shape change.
